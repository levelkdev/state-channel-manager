...
contract('ChannelManager', (accounts) => {
  ...
  it('should allow cooperative withdrawal from state channel', async () => {
    ...
    const channelId = await channelManager.getChannelId(...participants)

    // Make deposit into the channel for sender, receiver, arbiter.address
    await channelManager.deposit(...participants, { from: sender, value: INITIAL_DEPOSIT })
    ...
    // }
    channel = await channelManager.channels(channelId)
    ...
    await channelManager.requestWithdrawal(...participants, FINAL_NONCE, FINAL_CHANNEL_VALUE, { from: receiver })
    // Sender approves withdrawal
    await channelManager.completeWithdrawal(...participants, FINAL_NONCE, FINAL_CHANNEL_VALUE, { from: sender })
    ...
    await channelManager.requestWithdrawal(...participants, FINAL_NONCE, remainingDeposit, { from: sender })
    // Receiver approves withdrawal
    await channelManager.completeWithdrawal(...participants, FINAL_NONCE, remainingDeposit, { from: receiver })
